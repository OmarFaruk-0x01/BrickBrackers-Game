...
public class BrickableBlock : MonoBehaviour
{
    // Start is called before the first frame update
    private int BlockLife = GameManager.BrickableBlockLife;
    ...
    void OnCollisionEnter2D(Collision2D coliInfo)
    {
        if (coliInfo.collider.tag == "Ball")
        {
            ...
            if (BlockLife <= 0)
            {
                ...
                GameControler.Score += GameControler.BrickableBlockPerScore;
                GameControler.BrickableBlockNumber += 1;
                TriggerActive();
            }
            else if (BlockLife == 1)
            {
